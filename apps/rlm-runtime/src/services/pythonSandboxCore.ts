...
export type PythonSandboxCoreOptions = {
  budget?: Partial<SandboxBudget>;
  subAgentHandler?: SubAgentHandler;
  toolHandler?: ToolHandler;
};

export async function executePythonInProcess(input: {
  code: string;
  budget?: Partial<SandboxBudget>;
  subAgentHandler?: SubAgentHandler;
  toolHandler?: ToolHandler;
  ...
  const subAgentHandler = input.subAgentHandler;
  const toolHandler = input.toolHandler;
...
exec(${JSON.stringify(input.code)}, _rlm_user_globals, _rlm_user_globals)
`;
